...
async  function _initMove(){
    ...
    if(isHorizontal){
        ...
        if (autoPlay.value) { // 水平滚动设置warp width
              // 修正offsetWidth四舍五入
 ...
 function _move(){
    ...
    initData.reqFrame = requestAnimationFrame(function(){
        ...
        if(realSingleStopHeight){ //是否启动了单行暂停配置
            if (Math.abs(initData.yPos) % realSingleStopHeight < step) { // 符合条件暂停waitTime
          ...
          //  !! realSingleStopWidth: 设置的单步滚动距离默认0;
            ...
            if (Math.abs(initData.xPos) % realSingleStopWidth < step) { // 符合条件暂停waitTime
                initData.singleWaitTime = setTimeout(() => {
                    _move()
                }, waitTime)
            } else {
